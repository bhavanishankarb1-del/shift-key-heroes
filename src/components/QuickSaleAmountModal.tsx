...
interface QuickSaleAmountModalProps {
  open: boolean;
  onCancel: () => void;
  onConfirm: (amount: number) => void;
}

const QuickSaleAmountModal = ({ open, onCancel, onConfirm }: QuickSaleAmountModalProps) => {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Zap className="w-5 h-5 text-primary" /> Enter Quick Sale Amount
          </DialogTitle>
        </DialogHeader>

        <div className="flex flex-col items-center gap-5 py-2">
          <div className="w-full text-center py-6 rounded-xl bg-muted/40 border border-border">
            <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Sale Amount</p>
            <p className="text-4xl font-bold text-primary tabular-nums">${amount.toFixed(2)}</p>
          </div>
...
export default QuickSaleAmountModal;
